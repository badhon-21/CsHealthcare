...
namespace CsHealthcare.web.Areas.AppointmentSystem.Controllers
{
    public class AppointmentController : Controller
    {
        ...
        [HttpPost]
        //[AuthLog(Roles = "Administrator, MsAdmin, Application Admin, Patient Appointment")]
        public ActionResult SetAppointment(string PatientCode, string PatientName, string DoctorId, string PatientType, DateTime AppointmentDate, string AppointmentTime, string AppointmentType, string MobileNumber, string Location)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (val != null)
                    {
                        ...
                        if (AppServices.DoctorAppointmentRepository.GetData(gd => gd.PatientId == PatientId && gd.Date == AppointmentDate).Count() == 0)
                        {
                            ...
                            AppServices.DoctorAppointmentRepository.Insert(doctorAppointment);
                            AppServices.Commit();
        ...
        public ActionResult AppolintmentList(DateTime AppointmentDate, string DoctorId)
        {
            try
            {
                var AppointmentList = AppServices.DoctorAppointmentRepository.Get()
                    .Where(w => w.Date == AppointmentDate && w.DoctorId == DoctorId)
        ...
        //[AuthLog(Roles = "Administrator, MsAdmin, Application Admin, Patient Appointment")]
        public ActionResult DeleteAppointment(int AppointmentId, DateTime appDate, string DoctorId)
        {
            try
            {
                DoctorAppointment doctorsAppoinment = AppServices.DoctorAppointmentRepository.Get()
                    .Where(e => e.Id == AppointmentId).FirstOrDefault();

                if (doctorsAppoinment != null)
                {
                    AppServices.DoctorAppointmentRepository.Delete(doctorsAppoinment);
                    AppServices.Commit();
                }

                var drugList = AppServices.DoctorAppointmentRepository.Get().Where(w => w.Date == appDate && w.DoctorId == DoctorId)
                    .OrderBy(ob => ob.CreatedDate);
